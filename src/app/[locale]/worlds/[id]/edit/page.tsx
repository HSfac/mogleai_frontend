...
export default function EditWorldPage() {
  ...
  const [formData, setFormData] = useState<UpdateWorldDto>({
    name: '',
    description: '',
    setting: '',
    rules: [],
  ...
  const loadWorld = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        isAdultContent: world.isAdultContent,
      });
    } catch (error) {
  ...
  const handleChange = (field: keyof UpdateWorldDto, value: any) => {
          ...
          >
            <Stack spacing={3}>
              ...
              <TextField
                ...
                helperText={`${formData.name?.length || 0}/50`}
                sx={{
